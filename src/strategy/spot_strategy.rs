...
impl SpotStrategy {
    ...
    fn push_trade(&mut self, entry_price: f64, exit_price: f64, profit_percent: f64,
                  pnl_usdt: f64, bars: usize, exit_reason: &str, side: &str) {
    ...
    /// Mark-to-market equity: wallet value + open short unrealised PnL.
    ...
    pub fn max_drawdown_pct(&self) -> f64 { self.calculate_max_drawdown() }

    pub fn final_equity(&self, last_price: f64) -> f64 {
        self.mark_to_market_equity(last_price)
    }

    pub fn drawdown_stop_active(&self) -> bool { self.drawdown_stop_active }
    ...
    fn refresh_drawdown_state(&mut self, current_price: f64) {
        ...
        if self.peak_equity == 0.0 { return; }
    ...
    fn compute_signal_state(
        &self, current_price: f64, z_score: f64, ema_value: f64, in_cooldown: bool,
        rsi_val: f64, vol_val: f64, vol_sma_val: f64,
    ) -> SignalState {
        ...
        // Вхід у ЛОНГ: тільки якщо ціна вище EMA + перепроданість по Z
        let bullish_cross = bullish_trend && z_score < self.config.z_entry && rsi_ok_long && vol_ok;
        // Вхід у ШОРТ: тільки якщо ціна нижче EMA + перекупленість по Z
        let bearish_cross = bearish_trend && z_score > self.config.short_z_entry && rsi_ok_short && vol_ok;

        let no_signal_reason = if self.is_holding_asset { "HOLDING_LONG".into() }
        else if self.is_short { "SHORT_ACTIVE".into() }
        else if in_cooldown { "COOLDOWN".into() }
        else if !bullish_trend { "NOT_IN_UPTREND".into() }
        else if !bullish_cross { format!("Z_ENTRY_FAILED {:.2}", z_score) }
        else { "READY".into() };
    ...
    fn try_enter_position(&mut self, current_price: f64, atr_value: f64, signal: SignalState, in_cooldown: bool) -> Action {
        if !signal.bullish_trend || !signal.bullish_cross { return Action::NoSignal; }

        if self.is_holding_asset || in_cooldown || self.drawdown_stop_active { return Action::NoSignal; }
        ...
        let position_usdt = (risk_amount / (stop_distance / current_price)).min(self.wallet.usdt_balance * 0.98);

        if position_usdt < self.wallet.filters.min_notional { return Action::NoSignal; }
    ...
    fn try_enter_short(&mut self, current_price: f64, atr_value: f64, signal: &SignalState) -> Action {
        if !signal.short_entry { return Action::NoSignal; }

        let stop_distance = atr_value * self.config.short_stop_atr_mult;
        let margin_usdt = (self.current_equity * self.config.default_risk_per_trade_pct) / (stop_distance / current_price);

        if margin_usdt < self.wallet.filters.min_notional { return Action::NoSignal; }
    ...
    fn execute_short_exit(&mut self, exit_price: f64, reason: ExitReason, current_price: f64) -> Action {
        ...
        self.push_trade(self.short_entry_price, exit_price, pnl_pct, pnl_usdt, self.bars_in_position, reason.as_str(), "SHORT");
    ...
    fn try_close_short(&mut self, candle: &Candle) -> Option<(f64, ExitReason)> {
        ...
        if candle.high >= self.short_stop_price {
    ...
    fn execute_exit(
        ...
        if let Some(executed_price) = self.wallet.sell_all(trigger_price, false) {
            ...
            self.push_trade(ep, executed_price, profit_percent, pnl_usdt, bars, reason.as_str(), "LONG");
    ...
    pub fn finalize_backtest(&mut self, current_price: f64) {
        if self.is_holding_asset {
            let _ = self.execute_exit(
                ...
                ExitReason::EndOfData, Phase::PostSell, "BACKTEST_END",
            );
        }
        if self.is_short {
    ...
    fn position_type(&self) -> PositionType {
        if self.is_short          { PositionType::Short }
        else if self.is_holding_asset { PositionType::Long }
        else                      { PositionType::None  }
    }
    ...
    pub fn compute_backtest_result(&self, last_price: f64, csv_file: &str) -> BacktestResult {
        ...
        let winning: Vec<&Trade> = self.trade_history.iter().filter(|t| t.pnl_usdt > 0.0).collect();
        let losing:  Vec<&Trade> = self.trade_history.iter().filter(|t| t.pnl_usdt <= 0.0).collect();
        ...
        // Annualise per hourly candle (8 760 h/year)
        let annualisation = 8760.0_f64.sqrt();
        let sharpe_ratio  = if std_dev > 0.0 { (mean_ret / std_dev) * annualisation } else { 0.0 };
        ...
        for t in &self.trade_history {
            if t.pnl_usdt < 0.0 {
                streak += 1;
                if streak > max_consecutive_losses { max_consecutive_losses = streak; }
            } else { streak = 0; }
        }
    ...
    pub fn print_backtest_summary(&self, last_price: f64) {
        ...
        let long_n  = self.trade_history.iter().filter(|t| t.side == "LONG").count();
        let short_n = self.trade_history.iter().filter(|t| t.side == "SHORT").count();
        let total   = self.trade_history.len();
        let wins    = self.trade_history.iter().filter(|t| t.pnl_usdt > 0.0).count();
        let total_pnl: f64 = self.trade_history.iter().map(|t| t.pnl_usdt).sum();

        let long_pnl: f64  = self.trade_history.iter().filter(|t| t.side == "LONG").map(|t| t.pnl_usdt).sum();
        let short_pnl: f64 = self.trade_history.iter().filter(|t| t.side == "SHORT").map(|t| t.pnl_usdt).sum();

        println!("\n====== BACKTEST SUMMARY ======");
        println!("Total trades : {} (Long: {} ${:.2}  |  Short: {} ${:.2})",
        ...
        println!("Final equity : ${:.2}", self.mark_to_market_equity(last_price));
        println!("==============================\n");
    }
}
...
impl TradingStrategy for SpotStrategy {
    ...
    fn final_equity(&self, current_price: f64) -> f64 { self.mark_to_market_equity(current_price) }
    fn total_trades(&self) -> usize { self.trade_history.len() }
    ...
    fn on_candle_close(&mut self, candle: &Candle) {
        ...
        let (slow_value, fast_value, trend_value, rsi_value, atr_value, macro_ema_value, vol_sma_value) =
            match indicators {
                Some(v) => v,
                None    => return,
            };
        ...
        // FIX: pass all required arguments — previously rsi_val/vol_val/vol_sma_val were missing
        let signal = self.compute_signal_state(
            current_price, z_score, ema_value, in_cooldown,
            rsi_value, candle.volume, vol_sma_value,
        );
        ...
        if matches!(action, Action::NoSignal) && self.log_debug() {
            println!(
                ...
                signal.no_signal_reason, signal.short_no_signal_reason,
                z_score, self.config.ema_period, ema_value
            );
        }
        ...
        let (unrealized_pnl_usdt, unrealized_pnl_pct) = if self.is_holding_asset {
            ...
            let p = if self.entry_equity > 0.0 { u / self.entry_equity * 100.0 } else { 0.0 };
            (u, p)
        } else if self.is_short {
            let u = self.short_unrealized_pnl_usdt(current_price);
            let p = if self.short_margin_usdt > 0.0 { u / self.short_margin_usdt * 100.0 } else { 0.0 };
            (u, p)
        } else { (0.0, 0.0) };
        ...
        let total_pnl_pct  = if self.initial_capital > 0.0 { total_pnl_usdt / self.initial_capital * 100.0 } else { 0.0 };

        let position_exposure_usdt = if self.is_holding_asset {
            self.wallet.crypto_balance.max(0.0) * current_price
        } else if self.is_short { self.short_margin_usdt } else { 0.0 };
        ...
        // ── cooldown tickers ─────────────────────────────────────────────────
        if self.cooldown_bars_remaining > 0  { self.cooldown_bars_remaining -= 1; }
        if self.short_cooldown_bars     > 0  { self.short_cooldown_bars    -= 1; }
